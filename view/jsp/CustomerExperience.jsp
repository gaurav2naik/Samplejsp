...
<body onload="loadGraph()" id="ce">
	...
	<script>
		function checkFormSubmitData() {
			...
			if (status == "noDevice") {
				$("#err").html("<h6 style='color:#0A29B0'>No Device Selected</h6>");
				//alert("No new Devices to Add!!!")
				return false;
			} else if (status == "select") {
				$("#err").html("<h5 style='color:red;margin-top:0'>Please Select Some Device</h5>");
		...
		//	alert("hello")
			   ...
			   {
			    // alert("yes conatins -");
			     document.CustexpDeviceSelectionForm.action = "HotuserIOS.jsp?Device="+uuid ;
				 myform.submit();
			   }
			   else
				   
			   document.CustexpDeviceSelectionForm.action = "Hotuser.jsp?Device="+uuid ;
			   myform.submit();
			}
		</script>
</body>
